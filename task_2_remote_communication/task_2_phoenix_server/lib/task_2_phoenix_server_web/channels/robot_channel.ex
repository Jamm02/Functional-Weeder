defmodule Task2PhoenixServerWeb.RobotChannel do
  ...
  if (Map.fetch(message , :x) == 1) do
    left_value = 0
    if (Map.fetch(message, :y) == :a) do
      bottom_value = 0
      face_value = message.facing
      data = %{ "left" => left_value, "bottom" => bottom_value, "face" =>  face_value }
      Phoenix.PubSub.broadcast(Task2PhoenixServer.PubSub, "robot:update", data)

    else if (Map.fetch(message, :y) == :b) do
      bottom_value = 150
      face_value = message.facing
      data = %{ "left" => left_value, "bottom" => bottom_value, "face" =>  face_value }
      Phoenix.PubSub.broadcast(Task2PhoenixServer.PubSub, "robot:update", data)

    else if (Map.fetch(message, :y) == :c) do
      bottom_value = 300
      face_value = message.facing
      data = %{ "left" => left_value, "bottom" => bottom_value, "face" =>  face_value }
      Phoenix.PubSub.broadcast(Task2PhoenixServer.PubSub, "robot:update", data)

    else if (Map.fetch(message, :y) == :d) do
      bottom_value = 450
      face_value = message.facing
      data = %{ "left" => left_value, "bottom" => bottom_value, "face" =>  face_value }
      Phoenix.PubSub.broadcast(Task2PhoenixServer.PubSub, "robot:update", data)

    else if (Map.fetch(message, :y) == :e) do
      bottom_value = 600
      face_value = message.facing
      data = %{ "left" => left_value, "bottom" => bottom_value, "face" =>  face_value }
      Phoenix.PubSub.broadcast(Task2PhoenixServer.PubSub, "robot:update", data)
  ...
  else if (Map.fetch(message , :x) == 2) do
    left_value = 150
    if (Map.fetch(message, :y) == :a) do
      bottom_value = 0
      face_value = message.facing
  ...
  else if (Map.fetch(message , :x) == 3) do
    left_value = 300
    if (Map.fetch(message, :y) == :a) do
      bottom_value = 0
      face_value = message.facing
    ...
    else if (Map.fetch(message , :x) == 4) do
      left_value = 450
      if (Map.fetch(message, :y) == :a) do
        bottom_value = 0
        face_value = message.facing
    ...
    else if (Map.fetch(message , :x) == 5) do
      left_value = 600
      if (Map.fetch(message, :y) == :a) do
        bottom_value = 0
        face_value = message.facing
    ...
    is_obs_ahead = Task2PhoenixServerWeb.FindObstaclePresence.is_obstacle_ahead?(message["x"], Atom.to_string(:message["y"]), message["face"])
