 ...
 if (Enum.count(newk, &(&1 > 0)) <= 2) do
  ...
  def go_to_goal(
    ...
    if bool == true do
      ...
      robot
      # {:ok,robot}
    # else if robot.x == 2 and robot.y == :b  and robot.facing == :west do
    #   robot = right(robot)
    #   %Task4CClientRobotA.Position{x: x, y: y, facing: facing} = robot
    #   is_obs =
    #     Task4CClientRobotA.PhoenixSocketClient.send_robot_status(
...
# end
  ...
  def stop(robot, goal_locs, channel_status, channel_position, goalss,count) do
    ...
    goal_locs =
      if Enum.empty?(goal_locs) == false do
        ...
        # follow_line_until()
        # IO.inspect("rishikesh")
        # test_servo_a(60)
        if count == 1 do test_servo_a(60) end
        if count == 2 do test_servo_a(120) end
        if count == 3 do test_servo_a(180)end
        if count == 4 do test_servo_b(60)end
  ...
  def follow_line_until do
    ...
    start_follow_until(error, prev_error, difference, cumulative_error, correction, node_cnt, black_cnt)
  ...
  def start_follow_until(error, prev_error, difference, cumulative_error, correction, node_cnt, black_cnt) do
    readings =  get_sensor_readings()
   k = readings.og
   newk = readings.bin
   is_plant = get_ir
...
#  IO.inspect("#{newk}")
 ...
 black_cnt =
   if (black_cnt > 10) do
     ...
     stop()
     black_cnt
   else if (is_plant_new == true) do
    stop_new()
    drop_seed_new()
    start_follow(error, prev_error, difference, cumulative_error, correction, node_cnt, black_cnt)
    black_cnt

   else if (is_plant == true) do
     stop_new()
     drop_seed()
     start_follow(error, prev_error, difference, cumulative_error, correction, node_cnt, black_cnt)
     black_cnt
