defmodule ToyRobot do
  ...
  def correct_X(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, goal_x, goal_y, channel) do
    ...
    if(goal_x == x) do
      # is_obs = ToyRobot.PhoenixSocketClient.sendrobot_status(channel, robot)
    ...
    if x > goal_x do
      ...
      if (facing != :west) do
        robot = right(robot)
        %ToyRobot.Position{x: x, y: y, facing: facing} = robot
        # sendrobot_status(robot, channel)
...
def objInX_east(%ToyRobot.Position{x: x, y: y, facing: facing} = robot, goal_x, goal_y, channel, repeat) do
  ...
  if (y == :a) or (repeat == 1) do  #turn, move left and face west
    ...
    if(is_obs) do
      robot = left(robot)
      %ToyRobot.Position{x: x, y: y, facing: facing} = robot
      is_obs = is_obs = ToyRobot.PhoenixSocketClient.sendrobot_status(channel, robot)
      robot = move(robot)
      %ToyRobot.Position{x: x, y: y, facing: facing} = robot
      is_obs = ToyRobot.PhoenixSocketClient.sendrobot_status(channel, robot)
      robot = right(robot)
  ...
  Make a call to ToyRobot.PhoenixSocketClient.sendrobot_status/2
    ...
    if (y == goal_y) and (x == goal_x) do
      ...
      if y < goal_y do
        ...
        if (facing != :north) do
          ...
          if(is_obs = ToyRobot.PhoenixSocketClient.sendrobot_status(channel, robot)) do
  ...
  # def ToyRobot.PhoenixSocketClient.sendrobot_status(robot,channel)do
  #   current = self()
  #   pid = spawn_link(fn -> x = sendrobot_status(robot, channel)
  #                          send(current,x) end)
